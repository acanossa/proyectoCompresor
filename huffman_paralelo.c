...
// Comprime todos los archivos de un directorio en un archivo HUF
static int compress_directory(const char *directory, const char *archive_path) {
    ...
    /*
     * Crear un pipe y un proceso hijo para cada archivo.
     * El hijo realiza la compresión y el padre recibe el resultado.
     */
    int (*pipes)[2] = calloc(list.count, sizeof(int[2]));
    pid_t *children = calloc(list.count, sizeof(pid_t));
    ...
    // Crear los procesos hijos para comprimir los archivos
    for (size_t i = 0; i < list.count && result == 0; ++i) {
        
        // Crear el pipe para comunicarse con el hijo
        if (pipe(pipes[i]) != 0) {
        ...
        if (children[i] == 0) {
            ...
            // Crear archivo temporal donde se guardará la compresión
            ...
            // Comprimir el archivo utilizando Huffman
            int status = compress_file(source, list.items[i].path, temp);
            ...
            if (status == 0) {
                strncpy(
                    message.temp_path,
                    temp_path,
                    sizeof(message.temp_path)
                );

                message.temp_path[sizeof(message.temp_path) - 1] = '\0';

            } else {
                message.temp_path[0] = '\0';
                unlink(temp_path);
            }

            // Enviar el resultado de la compresión al padre
            write(pipes[i][1], &message, sizeof(message));
            close(pipes[i][1]);
    ...
    /*
     * Recibir los resultados de los procesos hijos
     * y agregar los archivos temporales al archivo HUF.
     */
    if (result == 0) {
        ...
        for (size_t i = 0; i < list.count; ++i) {
            ...
            if (append_file_to_archive(archive, message.temp_path) != 0) {
                result = -1;
                break;
            }

            unlink(message.temp_path);
        }
    }
