...
// Estructura para guardar el resultado de comprimir un archivo
typedef struct {
    int status;
    size_t index;
    char temp_path[4096];
...
// Estructura para guardar el resultado de extraer un archivo
typedef struct {
    int status;
...
// Información necesaria para cada proceso de extracción
typedef struct {
    pid_t child;
    int pipe_fd[2];
    char temp_path[4096];
...
// Agrega el contenido de un archivo temporal al archivo HUF
static int append_file_to_archive(FILE *archive, const char *temp_path) {
    ...
    // Copiar el archivo temporal en bloques de 8192 bytes
    while ((length = fread(buffer, 1, sizeof(buffer), temp)) != 0) {
        if (write_bytes(archive, buffer, length) != 0) {
            fclose(temp);
            return -1;
        }
    }

    int result = ferror(temp) ? -1 : 0;
...
// Comprime un archivo y escribe sus datos en el archivo HUF
static int compress_file(const char *source, const char *path, FILE *archive) {
    ...
    // Obtener frecuencias, tamaño y MD5 del archivo
    if (scan_file(source, frequencies, &size, digest) != 0) 
        return -1;
    ...
    if (tree.root >= 0) 
        make_codes(&tree, tree.root, codes, code_path, 0);

    // Calcular la cantidad total de bits del archivo comprimido
    for (int symbol = 0; symbol < 256; ++symbol)
        bit_count += frequencies[symbol] * strlen(codes[symbol]);

    uint16_t symbol_count = 0;

    // Contar la cantidad de símbolos diferentes utilizados
    for (int symbol = 0; symbol < 256; ++symbol) 
    ...
    // Escribir los metadatos del archivo en el archivo HUF
    if (path_length > MAX_PATH_LEN ||
        write_u16(archive, (uint16_t)path_length) != 0 ||
        write_bytes(archive, path, path_length) != 0 ||
        write_u64(archive, size) != 0 ||
        write_bytes(archive, digest, 16) != 0 ||
        write_u16(archive, symbol_count) != 0) {
        return -1;
    }

    // Escribir las frecuencias de los símbolos utilizados
    for (int symbol = 0; symbol < 256; ++symbol) {
        if (frequencies[symbol] == 0)
            continue;
        ...
        if (write_bytes(archive, &byte, 1) != 0 || write_u64(archive, frequencies[symbol]) != 0) {
            return -1;
        }
    }

    // Escribir la cantidad de bits y los datos comprimidos
    if (write_u64(archive, bit_count) != 0 || encode_file(archive, source, codes, bit_count) != 0) {
        return -1;
    }
...
// Obtiene la cantidad de procesadores disponibles
// tanto en macOS como en Debian
static size_t get_processor_count(void)
{
    return 4;
}


// Función ejecutada por cada hilo de compresión
static void *compression_worker(void *argument)
{
    CompressionContext *context = argument;

    while (1) {
        size_t index;
        pthread_mutex_lock(&context->mutex);

        if (context->next_index >= context->files->count) {
        ...
        CompressionResult *result = &context->results[index];

        result->index = index;
        result->status = -1;
        result->temp_path[0] = '\0';
        ...
        int length = snprintf(
            ...
            "%s/%s",
            context->directory,
            context->files->items[index].path
        );
        ...
        length = snprintf(
            result->temp_path,
            sizeof(result->temp_path),
            "/tmp/huf_thread_%ld_%zu.tmp",
            (long)getpid(),
            index
        );

        if (length < 0 ||
            (size_t)length >= sizeof(result->temp_path)) {
            result->temp_path[0] = '\0';
            continue;
        }

        FILE *temporary = fopen(result->temp_path, "wb");
...
// Comprime todos los archivos utilizando un grupo de hilos
static int compress_directory(
    const char *directory,
    ...
    if (collect_files(directory, "", &list) != 0) {
        free_file_list(&list);
        return 1;
    }

    size_t file_count = list.count;
    ...
    if (write_bytes(archive, MAGIC, 4) != 0 ||
        write_u32(archive, (uint32_t)file_count) != 0) {
        result = -1;
    }

    CompressionResult *results = NULL;
    ...
    // Esperar a que terminen todos los hilos creados
    ...
    /*
     * Los hilos ya terminaron. El hilo principal agrega
     * los temporales al archivo HUF en el orden original.
     */
    if (result == 0) {
        for (size_t i = 0; i < file_count; ++i) {
            if (results[i].status != 0 ||
                results[i].temp_path[0] == '\0') {
                result = -1;
                break;
            }

            if (append_file_to_archive(
                    archive,
                    results[i].temp_path
                ) != 0) {
                result = -1;
    ...
    // Eliminar todos los archivos temporales existentes
    for (size_t i = 0; i < file_count; ++i) {
        if (results != NULL &&
            results[i].temp_path[0] != '\0') {
            unlink(results[i].temp_path);
        }
    }
    ...
    if (result != 0) {
        fprintf(stderr, "Error al comprimir concurrentemente.\n");
        unlink(archive_path);
        return 1;
    }

    printf(
        "Comprimidos %zu archivos concurrentemente "
        "con %zu hilos en %s\n",
        file_count,
        thread_count,
        archive_path
...
// Descomprime un archivo y verifica que su contenido coincida con el MD5 original
static int extract_file(
    ...
    // Descomprimir los datos y escribirlos en el archivo de salida
    int decode_result = decode_file(
        compressed,
        tree,
        bit_count,
        size,
    ...
    // Abrir nuevamente el archivo para calcular su MD5
    ...
    // Leer el archivo en bloques y calcular el MD5
    while ((read_count = fread(buffer, 1, sizeof(buffer), check)) != 0)
    ...
    // Comparar el MD5 calculado con el almacenado en el archivo HUF
...
static int copy_data(FILE *archive, FILE *temp, uint64_t bit_count) {
    ...
    while (byte_count > 0) {
        size_t to_read = byte_count < sizeof(buffer)
                        ? (size_t)byte_count
                        : sizeof(buffer);

        if (read_bytes(archive, buffer, to_read) != 0)
            return -1;

        if (fwrite(buffer, 1, to_read, temp) != to_read)
            return -1;
...
static int extract_archive(
    ...
    for (uint32_t file_number = 0;
        ...
        if (length < 0 ||
            (size_t)length >= sizeof(task->output_path) ||
            make_parent_directories(task->output_path) != 0) {
            result = -1;
            break;
        }
